      ...
      <Card className="shadow-sm border-t-2 border-t-violet-500">
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between">
            ...
            <Badge
              variant="outline"
              className="text-violet-700 border-violet-300 bg-violet-50 gap-1.5 flex-shrink-0"
            >
        ...
        <CardContent className="space-y-6">
          {/* Two-column workspace: every source lives on the left so it's
              filled in as one combined run (not tabbed/exclusive); status,
              pipeline, and the run action stay pinned on the right. */}
          ...
          <div className="flex gap-4 flex-wrap">
            ...
            <div className="flex-1 min-w-[220px]">
              ...
              <Select
                ...
                items={environments.map((env) => ({ value: env.id, label: env.name }))}
              >
            ...
            <div className="flex gap-3 items-center flex-wrap">
              ...
              <Select
                ...
                items={references.map((r) => ({
                  value: r.id,
                  label: r.target_page ? `${r.file_name} — ${r.target_page}` : r.file_name,
                }))}
              >
          ...
          {/* Engine status — compact strip (replaces the old 5-big-card row)
              showing the same buildEngineCards(run) data live once a run
              exists. */}
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
              System Status
            ...
            <div className="space-y-1.5 rounded-lg border border-gray-100 bg-gray-50 p-3">
              {buildEngineCards(run).map((e) => (
                ...
                >
                  ...
                  <span className="flex items-center gap-1.5 text-xs font-medium text-gray-700 flex-shrink-0">
                    <span
                      className={`w-1.5 h-1.5 rounded-full inline-block ${e.dotColor}`}
                    />
                    {e.status}
                  </span>
                </div>
            ...
            <div className="flex flex-col gap-1.5">
              {PIPELINE_STEPS.map((step, i) => (
                <div key={step} className="flex items-center gap-2">
                  <span className="rounded-lg border border-gray-100 bg-gray-50 px-3 py-2 text-xs text-gray-700 flex-1">
                    {step}
                  </span>
